		...
		for skill_indx in range(0,len(skill_sequence)-1):
			...
			if hasattr(env, "obs_rms"):
				action = agent.select_action(np.hstack((env._normalize_shape(obs["observation"],env.obs_rms["observation"]),
													env._normalize_shape(desired_goal.reshape(1,4),env.obs_rms["achieved_goal"]),
				...
				action = agent.select_action(np.hstack((obs["observation"], desired_goal.reshape(1,4), oh_skill_indx.reshape(1,len(skill_sequence)))),
					deterministic=True,
				)
				value = agent.value(np.hstack((obs["observation"], desired_goal.reshape(1,4), oh_skill_indx.reshape(1,len(skill_sequence)))), action)
...
if (__name__=='__main__'):
	...
	env, eval_env, env_info = gym_vec_env('GHumanoidGoal_PB_BF-v0', num_envs)
	print("env = ", env)
	num_skills = None
