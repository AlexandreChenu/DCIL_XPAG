...
class DCILGoalSetterPB_variant_v4(GoalSetter, ABC):
	...
	def reset_done(self, env, observation, eval_mode=False):
		...
		## set skill
		do_reset_state = np.logical_and(is_done, np.logical_not(overshoot_possible)).astype(np.intc) ## state
		env.reset_before_reset_done(do_reset_state)
		env.set_state(reset_sim_states, do_reset_state)

		do_reset_max_episode_steps = is_done.copy() ## rollout budget
		env.set_max_episode_steps(reset_max_episode_steps, do_reset_max_episode_steps)

		do_reset_goal = is_done.copy() ## goal
		env.set_goal(reset_goals, do_reset_goal)
